...
uniform vec2 iResolution = vec2(204 ,350);
...
void fragment(  )
{
    ...
    // vec3 oricol = getPixelAt( vec2(q.x,q.y) ).xyz;
    vec3 col;
    ...
    // col.r = getPixelAt(vec2(x+uv.x+0.001,uv.y+0.001)).x+0.05;
    // col.g = getPixelAt(vec2(x+uv.x+0.000,uv.y-0.002)).y+0.05;
    // col.b = getPixelAt(vec2(x+uv.x-0.002,uv.y+0.000)).z+0.05;
    // col.r += 0.08*getPixelAt(0.75*vec2(x+0.025, -0.027)+vec2(uv.x+0.001,uv.y+0.001)).x;
    // col.g += 0.05*getPixelAt(0.75*vec2(x+-0.022, -0.02)+vec2(uv.x+0.000,uv.y-0.002)).y;
    // col.b += 0.08*getPixelAt(0.75*vec2(x+-0.02, -0.018)+vec2(uv.x-0.002,uv.y+0.000)).z;
